...
public sealed class DeletePostUseCaseTests
{
    [Theory]
    [InlineData(true)]
    [InlineData(false)]
    ...
    {
        var repo = new FakeRulesRepository { DeletePostResult = repoResult };
        var uc = new DeletePostUseCase(repo);

        var result = await uc.ExecuteAsync("s1", "p1");
